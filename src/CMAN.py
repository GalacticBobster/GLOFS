...
month = '9'
month_name = 'Sep'
os.makedirs("../../CMAN_summer", exist_ok=True)

for station in stations:
    station_lower = station.lower()
    #url = f"https://www.ndbc.noaa.gov/data/stdmet/{month_name}/{station_lower}{month}{year}.txt.gz"
    url = f"https://www.ndbc.noaa.gov/data/stdmet/{month_name}/{station_lower}.txt"
    fname = f"{station_lower}h{year}.txt"
    print(f"Downloading : {fname}")
    response = requests.get(url)

    if response.status_code == 200:
     with open(f"../../CMAN_summer/{fname}", "wb") as f:
